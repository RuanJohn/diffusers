...
class StableUnCLIPImg2ImgPipelineImageMerger(DiffusionPipeline, TextualInversionLoaderMixin):
    """
    ...
    def __call__(
        ...
        # Combine all image embeds into one tensor with equal weight for 
        # each image
        if len(all_image_embeds) > 1:
            image_embeds = torch.stack(all_image_embeds).mean(0)
        else:
            image_embeds = all_image_embeds[0]
